...
- Never use `sudo`, `su`, or `ssh`. If an operation requires the use of these commands, ask the user to run them for you.
...
Always refer to the "unslop" skill before writing README, comments, or documentation. Also refer to it before communicating to the user. You do not need to refer to "unslop" for agent-to-agent communication like when writing a SKILL.md, sub-agent communication, etc.
...
Every function should at minimum have a docstring that explains what it does, what arguments it takes, and what it returns.
